...
public class SecondPartStarter implements WorldGeneratorPart {
    ...
    private void startZorsanFightEvent(final World world){
        ...
        world.addListener(new ZorsanFightEvent(Configuration.getDoubleProperty("quest.zorsan_skirmish_encounter.chance"), ships, Dialog.loadFromFile("dialogs/encounters/zorsan_skirmish_start.json")));
    ...
    private void fixDiplomacy(World world) {
        ...
        if(prop == 2){
            world.getReputation().setReputation(ZorsanGenerator.NAME, HumanityGenerator.NAME, Reputation.HOSTILE_REPUTATION);
