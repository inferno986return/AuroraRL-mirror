...
public class PlanetScanScreen implements Room {
    ...
    public void enter(World world) {
        ...
        myCamera = new Camera(0, 0, newTileWidth, newTileHeight, planet.getWidth(), planet.getHeight());
        myCamera.setTarget(new BasePositionable(planet.getWidth() / 2, planet.getHeight() / 2));
    }

    @Override
    public void update(GameContainer container, World world) {
        if (container.getInput().isKeyDown(Input.KEY_ENTER) || container.getInput().isKeyDown(Input.KEY_ESCAPE) || container.getInput().isKeyDown(Input.KEY_S)) {
